...
def dynamic_threshold_white_balance(img):
    ...
    while Y >= 0:
        num += yhistogram[Y]
        if num > 0.02 * ysum:  # 取前10%的亮点为计算值，如果该值过大易过曝光，该值过小调整幅度小
            key = Y
            break
        Y = Y - 1
    # print(key)
...
if __name__== '__main__':
    ...
    img = cv2.imread("./14_1140_to_1138_fake_B.png")
    real_img = cv2.imread("./14_1140_to_1138_real_a.png")
    real_img_B = cv2.imread("./14_1140_to_1138_real_B.png")
    
    blur = cv2.GaussianBlur(real_img, (3, 3), 0)  # 用高斯滤波处理原图像降噪
    canny_real = cv2.Canny(blur, 50, 150)  # 50是最小阈值,150是最大阈值
    blur = cv2.GaussianBlur(img, (3, 3), 0)  # 用高斯滤波处理原图像降噪
    canny_fake = cv2.Canny(blur, 50, 150)  # 50是最小阈值,150是最大阈值
    
    blur = cv2.GaussianBlur(real_img_B, (3, 3), 0)  # 用高斯滤波处理原图像降噪
    canny_truth = cv2.Canny(blur, 50, 150)  # 50是最小阈值,150是最大阈值
    
    cv2.imwrite('canny_fake.png', canny_fake)
    cv2.imwrite('canny_real.png', canny_real)
    cv2.imwrite('canny_truth.png', canny_truth)
    
    #img_new = color_correction_of_image_analysis(img)
    #img_new = contrast_image_correction(img)
    #contours = cv2.findContours(img)
    img = np.array(0.6 * img + 0.4 * real_img, dtype = np.uint8)
    cv2.imwrite('./linear_combination.png', img)
     
    #blur = cv2.GaussianBlur(img, (3, 3), 0)  # 用高斯滤波处理原图像降噪
    #canny_combination = cv2.Canny(blur, 50, 150)  # 50是最小阈值,150是最大阈值
    #cv2.imwrite('./canny_linear_combination.png', canny_combination)
    
    img = mean_white_balance(img)
    img = perfect_reflective_white_balance(img)
    ...
    #img = gamma_trans(img)  # gamma变换
    #img7 = contrast_image_correction(img)
  
    b_gray, g_gray, r_gray = cv2.split(img)
    #b_gray = np.array(1.2 * b_gray, dtype = np.uint8)
    #g_gray = SSR(g_gray, size)
    #r_gray = SSR(r_gray, size)
    img = cv2.merge([b_gray, g_gray, r_gray])
    
    print('save transfered B')
    #cv2.imwrite('./B_1.png', img1)
    #cv2.imwrite('./B_2.png', img2)
    #cv2.imwrite('./B_3.png', img3)
    #cv2.imwrite('./B_4.png', img4)
    #cv2.imwrite('./B_5.png', img5)
    #cv2.imwrite('./B_6.png', img6)
    #cv2.imwrite('./B_7.png', img7)
    cv2.imwrite('./result.png', img)

    dst = np.zeros_like(img)
    cv2.normalize(img, dst, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U) 
    cv2.imwrite('./normalize.png', dst)

    img_hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
 
    turn_blue_hsv = img_hsv.copy()
    import ipdb
    ipdb.set_trace()

    turn_blue_hsv[:, :, 0] = turn_blue_hsv[:, :, 0] + 10
    turn_blue_img = cv2.cvtColor(turn_blue_hsv, cv2.COLOR_HSV2BGR)
    cv2.imwrite('./turn_blue.jpg', turn_blue_img)
